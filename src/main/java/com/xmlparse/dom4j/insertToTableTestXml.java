...
//	   while(list.hasNext()){
//		   PageInspectTable p=(PageInspectTable) list.next();
//		   System.out.println(p.getTagname()+"haha");
////		   List<PageInspectTable> al2=new ReadInspectTableTest().getValueByitemId(p.getId());
//		   
//	   }
		boolean b=false;
		...
		Element cEle=doc.addElement("check").addAttribute("inspecttype", p.getTname());
		...
		Element a3=a1.addElement("location").addAttribute("name", p1.getTagname());
		 List<PageInspectTable> cl=new ReadInspectTableTest().getT(p1.getTagname(),id);
